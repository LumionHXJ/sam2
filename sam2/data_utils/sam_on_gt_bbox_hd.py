...
import torch
from sam2.data_utils.utils import load_raw_annotations, calculate_stability_score,load_yolo_annotations

sam2_checkpoint = "/data/huxingjian/workspace/sam2/sam2_logs/configs/sam2.1_training/sam2.1_hiera_l_OBIMD_stage1/checkpoints/checkpoint.pt"
...
with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
    ...
    # load train_list
    with open("/data/huxingjian/workspace/sam2/data/OBIMD_hd_test_checked/train.txt") as f:
        train_list = [line.strip() for line in f.readlines()]

    # load raw data
    data_lookup = load_yolo_annotations(
        label_dir="data/OBIMD_hd_test_checked/labels",
        image_dir="data/OBIMD_hd_test_checked/images",
        image_ext=".jpg"
        )
    
    for image_id, path in tqdm(enumerate(train_list)):
        image = cv2.imread(os.path.join('data/OBIMD_hd_test_checked/images', path+'.jpg'))
        H, W, C = image.shape
        if os.path.exists(os.path.join('data/OBIMD_hd_test_checked/facsimile_json', path+'.json')):
            continue
        input_box = []
        ...
        for mask, score, stab_score, box in zip(masks, scores, stability_scores, input_box):
            ...
            pred_result['annotations'].append(dict(id=char_id, bbox=bbox.astype(int).tolist(), area=int(area), 
                                                   ...
                                                   crop_box=crop_box))
            char_id += 1
        with open(os.path.join('data/OBIMD_hd_test_checked/facsimile_json', path+'.json'), 'w') as f:
