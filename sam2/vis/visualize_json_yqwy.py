...
json_dir = "data_YQWY/sam_mining_stage2/facsimile_json_iou0.6"
rubbing_dir = "data_YQWY/raw/rubbing"
vis_dir = "data_YQWY/sam_mining_stage2/visualization"

random.seed(42)
file_list = random.sample(os.listdir(json_dir), 50) # fixed for same generation
...
for file in file_list:
    ...
    for i, annotation in enumerate(data['annotations']):
        ...
        x0, y0, w, h = bbox
        color = colors[i]
        image = cv2.rectangle(image, (int(x0), int(y0)), (int(x0+w), int(y0+h)), color, 2)
